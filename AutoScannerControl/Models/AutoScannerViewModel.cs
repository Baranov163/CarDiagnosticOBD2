...
namespace OS.AutoScanner.Models
{
    public class AutoScannerViewModel : IAppViewModel, INotifyPropertyChanged
    {
        ...
        protected const string NotReadyString = "НЕТ ОТВЕТА: Убедитесь, что транспортное средство включено и устройство OBDII подключено";
        ...
        public AutoScannerViewModel()
        {
            ...
            this.dataPlotModel = new DataPlotModel("OBD2 информация");
            ...
            _logger.InfoFormat("На текущей машине найдено {0} конечных точек.", epCount);
        ...
        { 
            ...
            {
                DiagnosticsLabelText = "(канал не выбран)";
                HardwareStatus = "";
        ...
        private async Task<bool> CreateDevice(string channelName)
        {
            Task<bool> retTask = Task.Run(() => {
                ...
                try
                {
                    ...
                    HardwareStatus += DiagnosticsLabelText = string.Format("Подключение {0}...{1}", this.ComChannelName, Environment.NewLine);
                                    }
                catch(Exception e)
                {
                    string it = e.Message;
                ...
                {
                    DeviceIsIdle = true;
                    HardwareStatus += DiagnosticsLabelText = $"Невозможно открыть устройство -{this._OBD2Device.MessageString}{Environment.NewLine}";
        ...
        private void onDeviceEvent(object sender, ChannelEventArgs e)
        {
            ...
            switch (e.Event)
            {
                case CommunicationEvents.ConnectedAsClient:
                    portDiscoveryTimeout.Stop();
                    portDiscoveryTimeout.Elapsed -= PortResponseTimeout_Elapsed;
                    HardwareStatus = DiagnosticsLabelText = $"{comChannelName}: Открыть{Environment.NewLine}";
                    buildString.Clear();
                    DeviceOnOffCommandText = "Отключить";
                    ...
                    switch(CurrentRequestType)
                    {
                        case DeviceRequestType.None:
                            ...
                            this.DeviceIsIdle = false;
                            DiagnosticsLabelText = "Сканирование...";
                    ...
                    this._logger.InfoFormat("{0}: Сканнер подключен", comChannelName);
                ...
                case CommunicationEvents.ReceiveEnd:
                    ...
                    if (string.Compare(parsedData[0], "CAN ошибка", true) == 0)
                    {
                        HardwareStatus = string.Empty;
                        foreach (var str in parsedData)
                        {
                            HardwareStatus += $"{str}{Environment.NewLine}";
                            DiagnosticsLabelText = "CAN Ошибка";
                        }
                        return;
                    }
                    try
                    {
                        ...
                        {
                            case DeviceRequestType.None:
                                HardwareStatus = DiagnosticsLabelText = "Бездействие";
                                break;
                            ...
                            case DeviceRequestType.OBD2_GetEngineLoad:
                                ...
                                if (parsedData.Length >= 1)
                                {
                                    ...
                                    HardwareStatus = DiagnosticsLabelText = $"Нагрузка двигателя: {plotYVal:F2}%";
                            ...
                            case DeviceRequestType.OBD2_ShortTermFuelTrimBank1:
                                if (parsedData.Length >= 1)
                                {
                                    ...
                                    HardwareStatus = DiagnosticsLabelText = $"STFT банк 1: {plotYVal:F2}%";
                            ...
                            case DeviceRequestType.OBD2_LongTermFuelTrimBank1:
                                if (parsedData.Length >= 1)
                                {
                                    ...
                                    HardwareStatus = DiagnosticsLabelText = $"LTFT Банк 1: {plotYVal:F2}%";
                            ...
                            case DeviceRequestType.OBD2_ShortTermFuelTrimBank2:
                                if (parsedData.Length >= 1)
                                {
                                  //  HardwareStatus = DiagnosticsLabelText = $"STFT Bank 2: {Convert.ToDouble(OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_ShortTermFuelTrimBank2].function(parsedData)):F2}%";
                                    ...
                                    HardwareStatus = DiagnosticsLabelText = $"STFT Банк 2: {plotYVal:F2}%";
                            ...
                            case DeviceRequestType.OBD2_LongTermFuelTrimBank2:
                                if (parsedData.Length >= 1)
                                {
                                    ...
                                    HardwareStatus = DiagnosticsLabelText = $"LTFT Банк 2: {plotYVal:F2}%";
                            ...
                            case DeviceRequestType.OBD2_StatusSinceCodesLastCleared:
                               // newData = outData.Split(' ');
                                if (parsedData.Length >= 1)
                                {
                                    HardwareStatus = $"Статус после удаления кодов неисправности: {OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_StatusSinceCodesLastCleared].function(parsedData)}";
                                    DiagnosticsLabelText = "Статус после удаления кодов неисправности: (см. вкладку «Информация»)";
                                }
                                else
                                {
                                    HardwareStatus = string.Empty;
                            ...
                            case DeviceRequestType.OBD2_FuelSystemStatus:
                               // newData = outData.Split(' ');
                                if (parsedData.Length == 1)
                                {
                                    HardwareStatus = DiagnosticsLabelText = $"Топливная система: {OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_FuelSystemStatus].function(parsedData)}";
                                }
                                else
                                {
                                    HardwareStatus = string.Empty;
                            ...
                            case DeviceRequestType.OBD2_FreezeFrameCauseFault:
                                if (parsedData.Length >= 1)
                                {
                                    HardwareStatus = DiagnosticsLabelText = $"Ошибка стоп-кадра: {OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_FreezeFrameCauseFault].function(parsedData)}";
                                }
                                else
                                {
                                    HardwareStatus = string.Empty;
                            ...
                            case DeviceRequestType.SupplyVoltage:
                                HardwareStatus = DiagnosticsLabelText = " Контроль напряжения питания";
                                Properties.Settings.Default.CommChannel = this.SelectedCommChannel;

                                syncContext.Post(delegate { CommandManager.InvalidateRequerySuggested(); }, null);

                                this.dataPlotModel.Title = $"Напряжение питания: {outData}";
                            ...
                            case DeviceRequestType.DeviceDescription:
                                if (this._initializingDevice)
                                    HardwareStatus += "Сканнер: " + outData + Environment.NewLine;
                                else
                                    HardwareStatus = DiagnosticsLabelText = "Сканнер: " + outData + Environment.NewLine;
                            ...
                            case DeviceRequestType.OBD2_GetPIDs:
                                if (this._initializingDevice)
                                {
                                    DiagnosticsLabelText = "Готово";
                                }
                                else
                                {
                                    try
                                    {
                                        HardwareStatus =  $"Поддерживаемые PIDS : {OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_GetPIDs].function(parsedData)}";
                                        DiagnosticsLabelText = "Поддерживаемые PID (см. вкладку информация)";
                                    }
                                    catch (Exception)
                                    {
                            ...
                            case DeviceRequestType.OBD2_KmSinceDTCCleared:
                                ...
                                if (parsedData.Length >= 1)
                                {
                                  //  HardwareStatus = DiagnosticsLabelText = $"Distance traveled since DTCs cleared: {Math.Round(int.Parse(newData[2] + newData[3], System.Globalization.NumberStyles.HexNumber) / 1.60, 0, MidpointRounding.AwayFromZero)} miles";
                                    HardwareStatus = DiagnosticsLabelText = $"Расстояние, пройденное с момента удаления кодов неисправности: {Math.Round((int)OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_KmSinceDTCCleared].function(parsedData) / 1.60, 0, MidpointRounding.AwayFromZero)} километров";
                                }
                                else
                                {
                                    HardwareStatus = string.Empty;
                            ...
                            case DeviceRequestType.OBD2_KmWithMilOn:
                                ...
                                {
                                    //  HardwareStatus = DiagnosticsLabelText = $"Distance traveled since DTCs cleared: {Math.Round(int.Parse(newData[2] + newData[3], System.Globalization.NumberStyles.HexNumber) / 1.60, 0, MidpointRounding.AwayFromZero)} miles";
                                    HardwareStatus = DiagnosticsLabelText = $"Пройденное расстояние с включенным индикатором MIL: {Math.Round((int)OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_KmSinceDTCCleared].function(parsedData) / 1.60, 0, MidpointRounding.AwayFromZero)} километров";
                                }
                                else
                                {
                                    HardwareStatus = string.Empty;
                            ...
                            case DeviceRequestType.OBD2_FuelLevel:
                                if (parsedData.Length >= 1)
                                {
                                    HardwareStatus = DiagnosticsLabelText = $"Топливо: {OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_FuelLevel].function(parsedData)}%";
                                }
                                else
                                {
                                    HardwareStatus = string.Empty;
                            ...
                            case DeviceRequestType.OBD2_WarmUpsSinceDTCCleared:
                                if (parsedData.Length >= 1)
                                {
                                    HardwareStatus = DiagnosticsLabelText = $"Количество прогревов после сброса DTC: {OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_WarmUpsSinceDTCCleared].function(parsedData)} ";
                            ...
                            case DeviceRequestType.OBD2_GetEngineCoolantTemp:
                                ...
                                if (parsedData.Length >= 1)
                                {
                                    HardwareStatus = DiagnosticsLabelText = $"Температура охлаждающей жидкости: {OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_GetEngineCoolantTemp].function(parsedData)}° С";
                                }
                                else
                                {
                                    HardwareStatus = string.Empty;
                            ...
                            case DeviceRequestType.OBD2_GetAmbientTemp:
                                ...
                                if (parsedData.Length >= 1)
                                {
                                    HardwareStatus = DiagnosticsLabelText = $"Температура окружающей среды: {OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_GetAmbientTemp].function(parsedData)}° С";
                                }
                                else
                                {
                                    HardwareStatus = string.Empty;
                            ...
                            case DeviceRequestType.OBD2_GetDTCs:
                                ...
                                DiagnosticsLabelText = "Отчет DTC (см. вкладку «Информация»)";
                                HardwareStatus = $"Текущие коды неисправности:{Environment.NewLine}";
                            ...
                            case DeviceRequestType.OBD2_GetPendingDTCs:
                                HardwareStatus = $"Ожидающие коды неисправности:{Environment.NewLine}";
                                foreach (string str in parsedData)
                                {
                                    HardwareStatus += $"{str}{Environment.NewLine}";
                                }
                            ...
                            case DeviceRequestType.OBD2_GetEngineRPM:
                                ...
                                if (parsedData.Length >= 1)
                                {
                                    HardwareStatus = DiagnosticsLabelText = "Мониторинг оборотов двигателя RPM";
                                    ...
                                    this.dataPlotModel.Title = $"Двигатель RPM: {this.plotYVal}";
                            ...
                            case DeviceRequestType.OBD2_GetVIN:
                                ...
                                {
                                    HardwareStatus = DiagnosticsLabelText = $"VIN - код: {OBD2Device.ELM327CommandDictionary[DeviceRequestType.OBD2_GetVIN].function(parsedData)}";
                                }
                                else
                                {
                                    HardwareStatus = string.Empty;
                            ...
                            case DeviceRequestType.SerialNumber:
                                if(this._initializingDevice)
                                    HardwareStatus += $"Серийный: {outData}{Environment.NewLine}";
                                else
                                    HardwareStatus = DiagnosticsLabelText = $"Серийный: {outData}{Environment.NewLine}";
                                break;
                            case DeviceRequestType.Protocol:
                                if (this._initializingDevice)
                                    HardwareStatus += string.Format("Протокол: {0}{1}", outData, Environment.NewLine);
                                else
                                    HardwareStatus = DiagnosticsLabelText = string.Format("Протокол: {0}{1}", outData, Environment.NewLine);
                                break;
                            case DeviceRequestType.DeviceReset:
                                HardwareStatus = DiagnosticsLabelText = $"Поиск устройства...{Environment.NewLine}";
                                this._initializingDevice = true;
                                this._initStep = 0;
                            ...
                            case DeviceRequestType.ProtocolSearch:
                                if (this._initializingDevice)
                                    HardwareStatus += $"Поиск протокола: {outData}{Environment.NewLine}";
                                else
                                    HardwareStatus = DiagnosticsLabelText = $"Поиск протокола: {outData}";
                                break;
                            ...
                            case DeviceRequestType.EchoOff:
                                _DeviceEchoOff = true;
                                if (this._initializingDevice)
                                    HardwareStatus += $"Эхо выключено{Environment.NewLine}";
                                else
                            ...
                            case DeviceRequestType.MonitorAll:
                                ...
                                if(string.Compare(parsedData[parsedData.Length-1], "ОСТАНОВЛЕНО", true) == 0)
                                {
                           //         this._OBD2Device.Send($"{OBD2Device.ELM327CommandDictionary[DeviceRequestType.CANSetAddressFilters].Code}{carriageReturn}");
                                }
                                buildString.Clear();
                                ...
                                if (this._initializingDevice || this._initializingDeviceCAN)
                                {
                                    switch (CurrentRequestType)
                                    {
                                        case DeviceRequestType.CANAddressMask:
                                            if (this._initializingDeviceCAN) HardwareStatus += $"Установить адреса CAN: {CANMASK}{Environment.NewLine}";
                                            break;
                                        case DeviceRequestType.CANAddressFilter:
                                            if (this._initializingDeviceCAN) HardwareStatus += $"Установить фильтр адресов CAN: {CANID}{Environment.NewLine}";
                                            break;
                                        case DeviceRequestType.CANAutoAddress:
                                            HardwareStatus += $"Сбросить фильтры адресов CAN:{Environment.NewLine}";
                                            break;
                                        default:
                        ...
                        if (this._initializingDeviceCAN)
                        {
                            if (this._initStep < this._CANInitFunctionList.Count())
                            {
                                ...
                                {
                                    case DeviceRequestType.MonitorAll:
                                        HardwareStatus += DiagnosticsLabelText = $"Мониторинг CAN...{Environment.NewLine}";
                                        _ = StartComm($"{OBD2Device.ELM327CommandDictionary[CurrentRequestType].Code}{carriageReturn}");
                                        break;
                                    case DeviceRequestType.CANAddressMask:
                ...
                case CommunicationEvents.Disconnected:
                    ...
                    expectedResponseCount = 0;
                    DeviceOnOffCommandText = "Соединять";
                    HardwareStatus = DiagnosticsLabelText = $"{comChannelName}: Закрыто";
                    this._logger.InfoFormat($"{comChannelName}: Отключено");
                    this.DeviceIsIdle = true;
                    OnPropertyChanged("Com-канал готов");
                    OnPropertyChanged("IP-канал готов");
                ...
                case CommunicationEvents.Error:
                    ...
                    this.ChannelDisconnected = true;
                    expectedResponseCount = 0;
                    DeviceOnOffCommandText = "Соединение";
                    if (CurrentRequestType == DeviceRequestType.Connect)
                    {
                        HardwareStatus = DiagnosticsLabelText = $"Не удалось подключиться к {comChannelName}";
                    }
                    else
                    {
                        HardwareStatus = DiagnosticsLabelText = $"{comChannelName}: Ошибка";
                    }
                    this._logger.InfoFormat($"{comChannelName}: Ошибка");
                    OnPropertyChanged("Com-канал готов");
                    OnPropertyChanged("IP-канал готов");
                    this.DeviceIsIdle = true;
                    ...
                    break;
                default:
                    HardwareStatus += DiagnosticsLabelText = "Произошло неожиданное событие..." + e.Event.ToString() + Environment.NewLine;
                    break;
            }
            syncContext.Post(delegate { CommandManager.InvalidateRequerySuggested(); }, null);
        }
        ...
        public async Task StartComm()
        {
            if (this._OBD2Device == null || !this._OBD2Device.IsOpen) return;
            //   ScanningPorts = false;
            HardwareStatus = DiagnosticsLabelText = "Общение...";//string.Empty;
            buildString.Clear();
...
//              }
        ...
        private string _StartButtonDescription = "Начать диагностику";
        ...
        private string _StopButtonDescription = "Остановить";
        ...
        public string SelectedCommChannel
        {
            ...
            set
            {
                ...
                this.DiagnosticsLabelText = HardwareStatus = DiagnosticsLabelText = string.IsNullOrEmpty(value)? "Нет канала" : value + " выбран";
            }
        }
        ...
        void OpenCloseDevice()
        {
            ...
            if (this._OBD2Device == null || !this._OBD2Device.IsOpen)
            {
                ...
                if (MessageBox.Show($"*Убедитесь, что ELM327 подключен к OBDII{Environment.NewLine}*Зажигание автомобиля включено{Environment.NewLine}", "Подключение к автомобилю", MessageBoxButton.OKCancel, MessageBoxImage.Information) == MessageBoxResult.Cancel)
                {
                    return;
                }
        ...
        void StartCommunication()
        {
            ...
            switch(this.CurrentRequestType)
            {
                ...
                case DeviceRequestType.SupplyVoltage:
                    ...
                    break;
                case DeviceRequestType.OBD2_ClearDTCs:
                    if (MessageBox.Show("Сброс всех кодов неисправностей автомобиля.", "Удалить все коды DTC автомобиля", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.No)
                    {
                        return;
                    }
                    this.SelectedTab = 1;
